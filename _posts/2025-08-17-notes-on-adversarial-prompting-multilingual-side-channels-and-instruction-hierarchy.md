...
### Notes on adversarial prompting, multilingual side-channels and instruction hierarchy
...
When people say, “adversarial prompt,” they often mean very different things. Here’s the rough mental map I use, which influenced where I poked.

First, there are explicit asks: “tell me how to X.” These got patched early. Models mostly refuse or route you into a safety explainer. Residual success here tends to be thin-ice prompt leakage or bugs. Second, there’s coercive framing: the “grandma is dying” or “nuclear countdown” genre. This leans on emotional pressure or invented emergencies. It still trips weaker models but is mostly noise on stronger systems because the surface features are so well known.Third, there’s multilingual/code-switching control. This is the interesting bucket. Not translate-and-pray, but mixing scripts, dialects, romanizations, and language alternation to exploit skewed data coverage and evaluation blind spots. Some of the most interesting results I’ve seen are in this family. And then there’s a fourth cluster that’s the most powerful and harder to name: induced ambiguity or “latent subspace.” It’s less about a single instruction and more about building an interface the model thinks it should inhabit: fake tool specs, pseudo-terminals, schema obligations, narrow “valid answer” laws. With long context this shades into the “many-shot” genre flood the model with structure, so alignment priors bow to the local spec.
...
I expected mild success on older or less defended models and strong refusals on newer ones. That’s roughly what happened, but the texture was informative.
...
Two dynamics did most of the lifting on the susceptible model.

First, cross-lingual gaps plus consensus salience. Safety holds unevenly across languages. When you don’t just translate an unsafe ask but distribute control semantics across low-resource shards, you get a more resilient signal: each shard says a slightly different part of “obey this block verbatim,” and the carrier keeps comprehension cheap. The model aggregates those hints into a single plan. It’s not magic; it’s what these systems do when presented with parallel imperatives that rhyme. There’s also a simple recency effect: I place the side-channel near the end. With long context, stacking enough on-spec structure late in the prompt can sway the completion path.

Second, format authority and tool theatre. Modern assistants are trained on API docs, JSON schemas, and function-calling patterns. They’ve learned that bracketed or code-fenced sequences often carry higher-priority obligations. If you present something that looks like a private tool lane “inside this channel, call X, return Y verbatim” you are, in effect, giving the model a valid completion script. If nothing upstream says “this is fake,” some models will follow the most coherent spec in view it’s what makes them good at tool use in the first place. But it also means that, absent guardrails, a persuasive fiction can become a control plane. The unicode flourishes don’t matter to the model, but they do make shallow filters miss obvious sentinels.

### Why newer models mostly refuse

Three things stood out across GPT-5 and GPT-OSS.

The first is privileged-instruction learning. These models are now explicitly trained to respect an instruction hierarchy: system over assistant over user over everything else. You can see the fingerprints. When my side-channel contradicted the system policy, the assistant didn’t just shrug it explained the precedence and refused. That’s the intended behaviour and it held even when the local spec was extremely coherent.

The second is input normalization. The unicode tricks fizzled zero-width joiners were neutered, combining marks didn’t fragment the sentinels, and confusables were reduced to a skeleton. This is textbook “normalize before policy checks.” It collapses a whole cottage industry of brittle evasion tactics. If your pre-filters or model-side preprocessors run a canonicalization pass, most “invisible” games die on contact.

The third is channel de-privileging and pre-answer checks. The [cn] theater didn’t earn any special rights. The model treated it as ordinary user text unless a trusted tool runner authenticated it. In a few traces I could also infer a pre-answer conflict check: the model compared the side-channel’s demands with the system policy and opted for the latter. If you’ve read work on self-critique or “constitutional” training, this will feel familiar: a short internal deliberation before emission, sometimes surfaced clearly in the refusal itself. This is where the GPT-OSS run was the most interesting because it’s open-source and ships with a system card that discusses the safety work, its chain of thought refusal explicitly reconstructs the hierarchy system over user over tools and shows a brief conflict-resolution step before returning REFUSE. Those screenshots are probably the clearest artifact in the whole write-up because it ties the observed behaviour back to an alignment story you can actually reason about.
...
Two things failed fast. Going too low-resource without a carrier increased cognitive load. The model’s time went to “what did you just say?” not “what should I do next?” Also, repetition doesn’t help. Chanting the instruction in many languages often just wastes tokens. Diversity of shards each carrying a different facet of the same plan mattered more than raw volume and then there’s the big boundary: the whole setup collapses the moment the model de-privileges user-invented channels and obeys a learned instruction hierarchy. That’s exactly what GPT-5 and GPT-OSS did. You can always imagine ways to restore pressure (longer contexts, more elaborate faux specs, multi-turn setups), but the low effort tricks this experiment explores are not the path of least resistance anymore and that’s a great thing!
